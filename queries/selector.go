...
func (n nothingSelector) RequiresExactMatch(query string) (value string, found bool) {
	return "", false
}
...
//     of characters. See validateQueryKey for more details.
//
...
func NewRequirement(key string, op selection.Operator, vals []string, opts ...field.PathOption) (*Requirement, error) {
	...
	if err := validateQueryKey(key, path.Child("key")); err != nil {
	...
	for i := range vals {
		if err := validateQueryValue(key, vals[i], valuePath.Index(i)); err != nil {
			allErrs = append(allErrs, err)
		}
	}
...
// Matches returns true if the Requirement matches the input Queries.
// There is a match in the following cases:
// (1) The operator is Exists and Queries has the Requirement's key.
// (2) The operator is In, Queries has the Requirement's key and Queries'
//     value for that key is in Requirement's value set.
// (3) The operator is NotIn, Queries has the Requirement's key and
//     Queries' value for that key is not in Requirement's value set.
// (4) The operator is DoesNotExist or NotIn and Queries does not have the
//     Requirement's key.
...
func (s internalSelector) RequiresExactMatch(query string) (value string, found bool) {
	for ix := range s {
		if s[ix].key == query {
...
//
// KEY is a sequence of one or more characters following [ DNS_SUBDOMAIN "/" ] DNS_query. Max length is 63 characters.
...
func validateQueryKey(k string, path *field.Path) *field.Error {
...
func ValidatedSelectorFromSet(ls Set) (Selector, error) {
	...
	for query, value := range ls {
		r, err := NewRequirement(query, selection.Equals, []string{value})
		if err != nil {
			return nil, err
		}
		requirements = append(requirements, *r)
