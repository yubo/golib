...
import (
	...
	"github.com/yubo/golib/staging/metrics/printer"
...
// Config configures options for emitting metrics.
type Config struct {
	...
	prometheusReporter             prometheus.Reporter            `yaml:"-"`
	prometheusConfigurationOptions PrometheusConfigurationOptions `yaml:"-"`

	// printer reporter
	Printer bool `yaml:"printer"`
}

func (mc Config) String() string {
	return util.Prettify(mc)
}

// NewRootScope creates a new tally.Scope based on a tally.CachedStatsReporter
// based on the the the config.
func (mc *Config) NewRootScope() (tally.Scope, io.Closer, error) {
...
func (mc *Config) NewRootScopeAndReporters(opts NewRootScopeAndReportersOptions) (
	tally.Scope,
	io.Closer,
	...
	if mc.Printer {
		...
		r, err := printer.NewReporter(opts)
		if err != nil {
	...
	if mc.PrometheusReporter != nil {
		...
		onError := func(e error) {
			klog.Errorf("register metrics error %v", e)
		}
		...
		mc.prometheusReporter = r
		mc.prometheusConfigurationOptions = opts
...
func (mc *Config) NewRootScopeReporter(r tally.CachedStatsReporter) (tally.Scope, io.Closer) {
	var (
		prefix string
...
func (mc *Config) SampleRate() float64 {
...
func (mc *Config) ReportInterval() time.Duration {
