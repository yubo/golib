...
import (
	...
	"github.com/yubo/golib/util/mergepatch"
)
...
func CreateThreeWayJSONMergePatch(original, modified, current []byte, fns ...mergepatch.PreconditionFunc) ([]byte, error) {
	...
	addAndChangePatch, err := CreateMergePatch(current, modified)
	if err != nil {
		return nil, err
	}
	...
	deletePatch, err := CreateMergePatch(original, modified)
	if err != nil {
		return nil, err
	}
	// Only keep deletion
	...
	patch, err := MergePatch(deletePatch, addAndChangePatch)
	if err != nil {
		return nil, err
	}
